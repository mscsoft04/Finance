<!DOCTYPE HTML>
<html>
  <head>
    <title>Bill</title>
  </head>
  <body>
   <div class="container">
   <table class="" style="width: 100%; border: 1px solid #ccc;padding:0px;background-color: #fff;">
      <tbody>
        <tr>
          <td rowspan="4" style="padding: 10px;">
            <img src="./image/agnilogo.jpg" alt="Agni Logo" width="100">
          </td>
          <td><strong>AGNISWARI CHIT FUNDS PVT LTD</strong></td>
        </tr>
        <tr>
          <td>
            HEAD OFFICE <br/>
            4/411,NORTH MAIN ROAD, SETHIYATHOPE & POST ,BHUVANAGIRI TALUK ,608702<br/>
            04144 - 244381,9488624381
            <br/>
          </td>
        </tr>
      </tbody>
    </table>
    <table style="width: 100%; border: 1px solid #ccc;padding: 0px;background-color: #fff;border-collapse: collapse;">
      <tbody>
        <tr>
          <td style="padding: 5px;">Group/Tkt.No : </td>
          <td style="padding: 5px 10px;"><span>A60T2028G / 13</span></td>
          <td style="padding: 5px;">Bill Date : </td>
          <td style="padding: 5px 10px;">08/08/2017  12:C</td>
        </tr>
         <tr>
          <td style="padding: 5px;">Cust.Name  : </td>
          <td style="padding: 5px 10px;">Mr .MANIKANDAN  S ( 5 )</td>
          <td style="padding: 5px;">Bill Series No : </td>
          <td style="padding: 5px 10px;">OFFICE BILL / 13267</td>
        </tr>
         <tr>
          <td style="padding: 5px;">Inst. Amt : </td>
          <td style="padding: 5px 10px;"><span>3000.00   & 20</span> <span style="display: inline-block;padding-left: 60px;">Inst.No 20</span></td>
          <td style="padding: 5px;">Chit Value: </td>
          <td style="padding: 5px 10px;">60000.00</td>
        </tr>
         <tr>
          <td colspan="4" style="padding: 20px 0px; padding-bottom: 0px;border-collapse: collapse;">
            <table style="width: 100%; border: 1px solid #ccc; border-collapse: collapse;" >
             <tr style="border: 1px solid #ccc;">
              <td colspan="" style="border-right: 1px solid #ccc; padding: 5px;"></td>
              <td style="padding:5px;text-align: right;padding-right: 20px;">Rs.</td>  
              <td style="padding:5px;">Ps.</td>
             </tr>
             <tr>
               <td style="border-right: 1px solid #ccc;padding: 5px;">Bill Amount</td>
               <td style="text-align: right;padding: 5px;">9,020</td>
               <td>.00</td>
             </tr>
              <tr>
               <td style="border-right: 1px solid #ccc;padding: 5px;">Penalty Amount</td>
               <td style="text-align: right;padding: 5px;">0</td>
               <td>.00</td>
             </tr>
              <tr>
               <td style="border-right: 1px solid #ccc;padding: 5px;">Total Amount</td>
               <td style="text-align: right;padding: 5px;">9,020</td>
               <td>.00</td>
             </tr>
             <tr style="border-top: 1px solid #ccc;font-weight: bold;">
               <td style="border-right: 1px solid #ccc;padding: 5px;">Balance Amount</td>
               <td style="text-align: right;padding: 5px;">0</td>
               <td>.00</td>
             </tr>
            </table>
          </td>
        </tr>
        <tr style="padding-top: 15px;">
          <td colspan="4" style="border: 1px solid #ccc;border-collapse: collapse;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td style="padding: 5px">Rupees:</td>
                <td colspan="3"><span style="border-bottom: 1px dashed #ccc; display: inline-block;padding: 5px;width: 95%;">NINE THOUSAND TWENTY ONLY</span></td>
              </tr>
              <tr>
                <td style="padding: 5px">Pay Type: </td>
                <td colspan="3"  style="padding: 5px">Cash</td>
              </tr>
            </table>
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                <td></td>
                <td></td>
                <td></td>
                <td colspan="" style="text-align: center;padding: 5px;padding-right: 0px;padding-top: 30px;font-weight: bold;"> Suryas</td>
              </tr>
               <tr>
                <td colspan="3" style="text-align: left;padding: 5px;padding-top: 20px;font-size: 14px;"><small><strong>* The administration is not responsible for the money being built without receipt</strong></small></td>
                <td colspan="1" style="text-align: center;padding: 5px;padding-top: 20px;padding-right: 0px;"> Cashier / Bill Collector</td>
              </tr>
            </table>
          </td>
        </tr>
      </tbody>
    </table>
</div>
  
  </body>
</html>
