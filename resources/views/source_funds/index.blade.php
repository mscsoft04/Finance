...
@section('content')
          ...
          <div class="card-header">
          	<div class="inner-header">
            	<div class="fl_in_h"><h5>Source Of Funds</h5></div>
                <div class="fr_in_h">
                    <button class="btn btn-link btn-sm btn-global btn-dark btn-fl-r" id="ExportReporttoExcel" >
                        <i class="fa fa-file-excel-o" aria-hidden="true"></i>
                        <span>Export</span></button>
                <a class="btn btn-link btn-sm btn-global btn-blue btn-fl-r"  href="{{ route('sourceOfFunds.create') }}">
...
$(document).ready(function() {
  ...
  var editIcon = function ( data, type, full, meta ) {
   ...
   var table=  $('.subscriber').DataTable({
        ...
        "ajax": "{{ route('sourceOfFunds.getdata') }}",
        
        "columns":[
            {"data":"name"},
            {  "data": "id", render: function ( data, type, row ) {
                if ( type === 'display' ) {
                  var url = '{{ route("sourceOfFunds.edit", [":id"]) }}';
